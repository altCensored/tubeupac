...
from tubeupac.TubeUp import TubeUp, DOWNLOAD_DIR_NAME
...
def copy_testfiles_to_tubeupac_rootdir_test():
    ...
    testfiles_dir = os.path.join(current_path, 'test_tubeupac_files',
                                 'files_for_upload_and_download_tests')

    for filepath in os.listdir(testfiles_dir):
        shutil.copy(
            os.path.join(testfiles_dir, filepath),
            os.path.join(current_path, 'test_tubeupac_rootdir', 'downloads',
                         filepath))
...
class TubeUpTests(unittest.TestCase):
    ...
    def test_get_resource_basenames(self):
        ...
        expected_result = {os.path.join(
            current_path, 'test_tubeupac_rootdir', 'downloads',
            'KdsN9YhkDrY')}
