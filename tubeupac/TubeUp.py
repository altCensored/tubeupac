...
class TubeUp(object):
    ...
    def dir_path(self, dir_path):
        ...
        downloads_dir_path = os.path.join(extended_usr_dir_path, DOWNLOAD_DIR_NAME)

        if os.path.exists(extended_usr_dir_path) and not os.path.isdir(extended_usr_dir_path):
            raise TubeUp.DirError(
                'Download root path "%s" already exists as a file. '
                "Choose a different --dir to avoid data loss." % extended_usr_dir_path
            )

        if os.path.exists(downloads_dir_path) and not os.path.isdir(downloads_dir_path):
            raise TubeUp.DirError(
                'Download path "%s" already exists as a file. '
                "Choose a different --dir to avoid data loss." % downloads_dir_path
            )
        ...
        try:
            ...
            raise TubeUp.DirError(
                'Cannot create download directory "%s": %s' % (downloads_dir_path, exc)
            ) from exc
